    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#f8fafc' }}>
      <ScrollView contentContainerStyle={{ padding: 24 }}>
        <View style={{ backgroundColor: '#1e293b', borderRadius: 20, padding: 24, marginBottom: 20, alignItems: 'center' }}>
          ...
          <Text style={{ color: '#f59e0b', fontWeight: '700', textTransform: 'uppercase', letterSpacing: 1 }}>
            {role === 'hub_manager' ? 'Hub Manager' : 'Hub Staff'}
          </Text>
        </View>
...
export const HubStaffNavigator = () => (
  ...
  >
    <Tab.Screen name="HubDash" component={HubManagerDashboard}
      options={{ tabBarIcon: ({ focused }: any) => <TabIcon icon="📊" label="Dashboard" focused={focused} /> }}
    />
    <Tab.Screen name="HubQR" component={HubQRScreen}
      options={{ tabBarIcon: ({ focused }: any) => <TabIcon icon="📷" label="Scan QR" focused={focused} /> }}
    />
    <Tab.Screen name="HubProfile" component={ProfileTab}
